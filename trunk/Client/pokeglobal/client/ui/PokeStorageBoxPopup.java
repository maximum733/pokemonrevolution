...
public class PokeStorageBoxPopup extends Frame {
	...
	public PokeStorageBoxPopup(PacketGenerator out, String pokeList, OurPokemon[] playerTeam) throws SlickException {
		...
		for (int i = 0; i <= 29; i++){
			...
			} catch(NumberFormatException e){indexes[i] = 999;}
		}
	...
	public void layoutButtons(){
		...
		switchPoke.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				setVisible(false);
				TeamForBox teamPanel = new TeamForBox(team, boxNum, boxIndex, packetGen);
				getDisplay().add(teamPanel);
				teamPanel.setLocation(getDisplay().getWidth()/2-teamPanel.getWidth()/2,
						getDisplay().getHeight()/2 - teamPanel.getHeight()/2);
			}});
