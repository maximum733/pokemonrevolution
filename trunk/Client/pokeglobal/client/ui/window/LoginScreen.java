...
import pokeglobal.client.ui.ServerSelector;
import pokeglobal.client.ui.StarterChoices;
import pokeglobal.client.ui.ToSWindow;
	...
	private void initGUI() {
           ...
           try {
        	   ...
        	   if (now.get(Calendar.MONTH) >= 11) {
        		   if (now.get(Calendar.DAY_OF_MONTH) > 20 && now.get(Calendar.DAY_OF_MONTH) < 28) {
        			   GlobalGame.getSoundPlayer().playChannel("robotchristmas", "music");
        		   } else {
       ...
       private boolean validData() {
               if (starterChoices.getChoice() != -1) {
                       if (!authPass.getText().equals("") &&
                                       ...
                                       if (isEmailValid(regEmail.getText())) {
                                               if (!authUser.getText().equals("")) {
                                                       if (!authUser.getText().contains(" ")) {
                                                               ...
                                                               GlobalGame.messageBox("You may not have spaces in your username.", this);
                                                       }
                                               } else {
                               ...
                               GlobalGame.messageBox("Your password must be typed in both boxes.", this);
                       }
               } else {
                       GlobalGame.messageBox("Make sure your Pokemon is selected correctly.", this);
               }
               return false;
       }
