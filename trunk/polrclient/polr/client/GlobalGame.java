...
import polr.client.sound.SoundManager;
import polr.client.ui.ChatWindow;
import polr.client.ui.PartyInfo;
import polr.client.ui.PokeStorageBoxPopup;
import polr.client.ui.ShopWindow;
...
/**
 * GlobalGame launches the game client. It runs the init() method and
 * initialises everything needed. Then, it loops between update and render and
 * accepts input when given.
 */
public class GlobalGame extends BasicGame {
	...
	private static SoundManager soundPlayer;
	...
	public static void main(String args[]) {
		try {
			...
			// TODO: When packaging for webstart, turn this into a
			// WebstartMuffin
			// (no filename required for it, and Files don't work in WS)
			options = new FileMuffin().loadFile("options.dat");
			if (options == null) {
				options = new HashMap<String, String>();
				options.put("muteSound", Boolean.toString(false));
			}
			AppGameContainer container = new AppGameContainer(new GlobalGame());
			container.setDisplayMode(width, height, Boolean
					.parseBoolean(options.get("fullScreen")));
			container.start();
		} catch (Exception e) {
	...
	public void init(GameContainer g) throws SlickException {
		...
		soundPlayer.mute(Boolean.parseBoolean(options.get("muteSound")));
		...
		teamInfo = new PartyInfo(new OurPokemon[] { null, null, null, null,
				null, null }, packetGen, mainInterface);
		...
		/*
		 * intro = new IntroScreen(); display.add(intro);
		 */
	 ...
	 * 
	 * @param g
	 *            - GameContainer to be updated
	 * @param delta
	 *            - FPS of the GameContainer
	 */
	@Override
	public void update(GameContainer g, int delta) {
		...
		if (thisPlayer != null && thisPlayer.map != null
				&& playerQueue.size() > 0) {
		...
		if (newMap && isPlaying && loading.isVisible()) {
			...
			GameMap map;
			try {
				map = new GameMap("res/maps/" + (mapX) + "." + (mapY) + ".tmx",
						"res/maps");
			...
			int x = 0;
			if (mapX > -30 && mapY > -30) {
				// Only load surrounding maps if outdoors. All maps from -50,-50
				// to -31,-31 are indoors.
				for (int y = -1; y < 2; y++) {
					for (x = -1; x < 2; x++) {
						try {
							if (!(x == 0 && y == 0)) {
								map = new GameMap("res/maps/"
										+ String.valueOf(mapX + x) + "."
										+ String.valueOf(mapY + y) + ".tmx",
										"res/maps");
								map.setGraphics(g.getGraphics());
			...
			// Start playing the new route music
			getSoundPlayer().playChannel("route", "music", true);
	...
	public void setWildEnemy(String a, String b, String c, String d, String e) {
		getBattle().addEnemyPokemon(a, 0, Integer.parseInt(b),
				Integer.parseInt(c), Integer.parseInt(d), e);
			...
			for (int x = 0; x <= 2; x++) {
				for (int y = 0; y <= 2; y++) {
					...
					if (thisMap != null && thisMap.isRendering()) {
						thisMap
								.render(thisMap.getXOffset() / 2, thisMap
										.getYOffset() / 2, 0, 0, (arg0
										.getScreenWidth() - thisMap
										.getXOffset()) / 32, (arg0
										.getScreenHeight() - thisMap
										.getYOffset()) / 32, true);
					}
				}
			}
	...
	public static void loadDPFont() throws SlickException {
		dpFont = new AngelCodeFont("res/fonts/dp.fnt", "res/fonts/dp.png");
	...
	public void beginBattle(String enemyName, char type, int index) {
		 ...
		 * 
		 * String[] pokemonStats = battleData.split(";"); enemyPokes[0] = new
		 * TempPokemon(); enemyPokes[0].setName(pokemonStats[0]);
		 * enemyPokes[0].setSpecies(Enums.Pokenum.values()
		 * [Integer.parseInt(pokemonStats[1])]);
		 * enemyPokes[0].setType1(Enums.Poketype.valueOf( pokemonStats[2])); try
		 * { enemyPokes[0].setType2(Enums.Poketype.valueOf( pokemonStats[3])); }
		 * catch (Exception ex) { }
		...
		battle = new Battle(enemyName, packetGen, thisPlayer.getTeam(), index,
				this);
		if (type == 'p')
	...
	/**
	 * Connects to the game server. This method will adjust if a proxy was
	 * specified.
	 */
	public void connect() {
		try {
			...
			if (PROXY.equalsIgnoreCase("")) {
				...
				try {
					...
					final PrintWriter netOut = new PrintWriter(
							new OutputStreamWriter(gameSocket.getOutputStream()));
					int i = 0;
					...
					proxyHandle = new ProxiedProtocolHandler(this, netIn,
							netOut);
					new Thread(proxyHandle);
				} catch (SocketException e) {
					e.printStackTrace();
					JOptionPane.showMessageDialog(null, "Connection to port "
							+ PORT + " not allowed by proxy.");
					SERVER = "";
					return;
				} catch (Exception e) {
					e.printStackTrace();
	...
	public void useShop(HashMap<String, Integer> merch) {
		ShopWindow shop = new ShopWindow(packetGen, merch);
		getDisplay().add(shop);
	}

	public void usePokeStorageBox(String pokeList) throws SlickException {
		PokeStorageBoxPopup storageBoxPopup = new PokeStorageBoxPopup(
				packetGen, pokeList, thisPlayer.getTeam());
		getDisplay().add(storageBoxPopup);
	...
	public void mouseClicked(int button, int x, int y, int clickCount) {
		...
		if (speechPopup != null
			 ...
			 */
			/*
			 * } else if (getLogin() != null && getLogin().getSpeechy() != null
	...
	public void keyPressed(int key, char c) {
		...
		if (isPlaying
				...
				&& !newMap
				&& (playerQueue.size() == 0 && thisPlayer.map.getMapPlayers()
						.size() > 0)) {
			if (!mainInterface.isChatting() && (key == (Input.KEY_RSHIFT))) {
				...
				// getReqWindow().clear();
			} else if ((key == (Input.KEY_SPACE) || key == (Input.KEY_RCONTROL))
					&& !thisPlayer.isAnimating()
				...
				if (thisPlayer.facing == Dirs.Up) {
					packetGen.write("TT" + GlobalGame.CHARSEP + thisPlayer.x
							+ GlobalGame.CHARSEP + (thisPlayer.y - 32));
				} else if (thisPlayer.facing == Dirs.Down) {
					packetGen.write("TT" + GlobalGame.CHARSEP + thisPlayer.x
							+ GlobalGame.CHARSEP + (thisPlayer.y + 32));
				} else if (thisPlayer.facing == Dirs.Left) {
					packetGen.write("TT" + GlobalGame.CHARSEP
							+ (thisPlayer.x - 32) + GlobalGame.CHARSEP
							+ thisPlayer.y);
				} else if (thisPlayer.facing == Dirs.Right) {
					packetGen.write("TT" + GlobalGame.CHARSEP
							+ (thisPlayer.x + 32) + GlobalGame.CHARSEP
