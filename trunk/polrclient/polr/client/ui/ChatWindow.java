...
public class ChatWindow extends Frame {
	...
	private void initGUI() {
		...
		try {
			dpFont = GlobalGame.getDPFontSmall();
			setTitle("Chat");
	...
	public void appendText(String newChat) {
		...
		newChat = newChat.substring(0, endex + 1)
				+ newChat.substring(endex + 1)
						.replace(">", " is greater than ").replace("<",
								" is less than ");
	...
	private void checkChatWindow() {
		try {
			if (chatList.getLineCount() >= ((int) getHeight() - 48)
					/ dpFont.getLineHeight()) {
				String[] s = chatList.getLinesAsText();
				chatList.setCaretPosition(0);
				chatList.setText("");
				// this next line causes the window to be unresizable
				// we need a way to calculate the max possible amount of lines
				// in resizeable window can fit
				int adj = (int) ((s.length - (getHeight()
						- getTitleBar().getHeight() - chatType.getHeight())
						/ dpFont.getLineHeight()) + 1);
