...
public class BagPane extends Container {
	...
	public BagPane(GlobalGame game) {
		super();
		thisGame = game;
		m_use = new Button[14];
	...
	public void updateItems(List<Item> m_items) {
		...
		for (int i = 0; i < 7 && i < m_items.size(); i++) {
			...
			if (m_items.get(i).pokeSelect()) {
				...
				m_use[i].addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						GlobalGame.getPacketGenerator().write("Bu" + itemNo);
					}
				});
			}
			this.add(m_use[i]);
			...
			m_drop[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					GlobalGame.getPacketGenerator().write("Bd" + itemNo);
				}
			});
			this.add(m_drop[i]);
		}
		...
		for (int i = 0; i < this.getParent().getDisplay().getChildren().length; i++) {
			try {
				if (this.getDisplay().getChildren()[i].getName()
						.equalsIgnoreCase("Pokemon Selector")) {
					pokeSelect = (UseItemPokemonWindow) this.getDisplay()
							.getChildren()[i];
