...
/*
 * The request window is where requests for battles and trades are viewed and made.
 */
public class RequestWindow extends Frame {
	...
	public Button tradeCancel, tradeAccept, tradeDecline, tradeSend,
			incPokeIndex, decPokeIndex, incAmount, decAmount;
	int pokeIndex;
	int pokedollars;
	...
	public void initGUI() {
		...
		tradeSend.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				GlobalGame.packetGen.write("to" + "," + pokedollars + ","
						+ (pokeIndex + 1));
	...
	public boolean contains(String username) {
		...
		for (int i = 0; i < playerInfo.size(); i++) {
			if (playerInfo.get(i).getText().substring(0,
					playerInfo.get(i).getText().indexOf(" L:")) == username) {
				result = true;
				break;
			}
		}
	...
	public void addRequest(final String username, String request) {
		if (request.charAt(0) == 'f') {
			...
			String s = "c" + username.toString() + ","
					+ request.substring(request.indexOf(',') + 1);
			Offer r = new Offer(username, s, request.substring(1, request
					.indexOf(',')));
			if (m_offers.size() > 4) {
				m_offers.remove(0);
				m_offers.trimToSize();
			...
			if (username.length() > 7)
				l.setText(username.substring(0, 8) + " L: "
						+ request.substring(1));
			else
			...
			b.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					GlobalGame.packetGen.write("c" + username.toString() + ","
							+ pokedollars);
					setEnabled(false);
				}
			});
