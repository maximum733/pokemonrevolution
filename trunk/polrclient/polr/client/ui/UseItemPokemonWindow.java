...
public class UseItemPokemonWindow extends Frame {
	...
	public void openMoveSelect(String packet, int index) {
		...
		for (int i = 0; i < this.getParent().getDisplay().getChildren().length; i++) {
			try {
				if (this.getDisplay().getChildren()[i].getName()
						.equalsIgnoreCase("Move Selector")) {
					moveSelect = (UseItemMoveWindow) this.getDisplay()
							.getChildren()[i];
	...
	public UseItemPokemonWindow(final int itemID, boolean moveSelect,
			OurPokemon[] pokes) {
		super();
			...
			if (pokes[i] != null) {
				...
				if (moveSelect) {
					...
					case 0:
						select[i].addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								GlobalGame.getPacketGenerator().write(
										"Bu" + itemID + "," + 0);
								close();
							}
						});
