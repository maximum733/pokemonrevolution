...
import polr.client.ui.base.layout.LayoutManager;
 ...
 * methods make calls to the content pane for convenience: add, remove,
 * setLayout, setBackground and remove. It's good practice to always call
 * getContentPane() first when dealing with the window's children.
 * 
 * @author davedes
 */
public class Frame extends Window {
	...
	/**
	 * Allows users to add/remove components directly to the window's container.
	 * Set to <tt>true</tt> if we wish to forward calls to the content pane,
	 * false if we wish the calls to directly affect this container. Calls that
	 * are affected are add, remove, setBackground, getBackground.
	 * 
	 * @param enabled
	 *            <tt>true</tt> if we wish to forward calls to the content pane
	 *            (the default)
	 */
	 ...
	 * 
	 * @return the scroll bar appearance set for this component 1.5 feature
	 *         public FrameAppearance getAppearance() { return
	 *         (FrameAppearance)super.getAppearance(); }
	 */

	/**
	 * Sets the appearance for this slider. If <code>appearance</code> is not an
	 * instance of FrameAppearance, an <code>IllegalArgumentException</code> is
	 * thrown.
	 * 
	 * 
	 * @param appearance
	...
	public void setAppearance(ComponentAppearance appearance) {
		if (!(appearance instanceof FrameAppearance))
			throw new IllegalArgumentException(
					"must pass instance of window appearance");
		super.setAppearance(appearance);
	}

	// TODO: rootpane checking so client can access actual component (rather
	// than just content pane)
	...
	/**
	 * Sets the background color for the window's content pane.
	 * 
	 * @param c
	 *            the content pane color
	 */
	...
	/**
	 * Gets the background color for the window's content pane.
	 * 
	 ...
	 * 
	 * @param text
	 *            the text for this window's title bar
	 */
	...
	void updateResizerY() {
		// HACK: get rid of offset hacks in updateResizerX, updateResizerY and
		// setWidth
		if (resizer != null)
	...
	public void setContentPane(Container cp) {
		if (cp == null)
			throw new IllegalArgumentException(
					"Can't have a null content pane!");
		this.contentPane = cp;
		contentPane.setFocusable(true);
	}
	...
	public static class TitleBar extends Label {
		...
		void updateWindowIcon() {
			if (windowIcon.getImage() != null) {
				...
				windowIcon.setX(2);
				windowIcon.setY(this.getHeight() / 2f - windowIcon.getHeight()
						/ 2f);
		...
		void updateCloseButtonLocation() {
			closeButton.setX(getWidth() - closeButton.getWidth()
					- getPadding().right);
			closeButton.setY(this.getHeight() / 2f - closeButton.getHeight()
					/ 2f);
		}
		...
		public boolean isBorderRendered() {
			return getWindow() != null ? getWindow().isBorderRendered() : super
					.isBorderRendered();
		}
	}
			...
			if (b == MouseEvent.BUTTON1) {
				...
				if (minWidth == MAX_RESIZE || abX >= minWidth)
					if (maxWidth == MAX_RESIZE || abX < maxWidth)
						window.setWidth(abX);
				if (minHeight == MAX_RESIZE
						|| abY - titleBar.getHeight() >= minHeight)
					if (maxHeight == MAX_RESIZE
							|| abY - titleBar.getHeight() < maxHeight)
