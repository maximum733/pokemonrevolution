...
public class ThemeDump extends BasicGame {
	...
	public void init(GameContainer container) throws SlickException {
		...
		for (int i = 0; i < labels.length; i++) {
			...
			methodNames[i].setLocation(startx, starty + i
					* (methodNames[i].getHeight() + 5));
			parent.add(methodNames[i]);

			labels[i] = new BorderLabel();
			labels[i].setOpaque(true);
			labels[i].setSize(65, 25);
			labels[i].setLocation(startx + methodNames[i].getWidth() + 5,
					starty + i * (labels[i].getHeight() + 5));
			parent.add(labels[i]);
		}
		...
		if (Sui.getSkin() instanceof SimpleSkin) {
			...
			roundBox.setLocation(box.getX(), box.getY() - roundBox.getHeight()
					- 5);
...
/*
 * CODE import mdes.slick.sui.skin.ColorUIResource; import
 * mdes.slick.sui.theme.Theme; import org.newdawn.slick.Color;
 * 
 * private Color activeTitleBar1 = private Color activeTitleBar2 = private Color
 * background = private Color disabledMask = private Color foreground = private
 * Color primary1 = private Color primary2 = private Color primary3 = private
 * Color primaryBorder1 = private Color primaryBorder2 = private Color
 * secondary1 = private Color secondary2 = private Color secondary3 = private
 * Color secondaryBorder1 = private Color secondaryBorder2 = private Color
 * titleBar1 = private Color titleBar2 =
 * 
 * public String getName() { return ; } public Color getActiveTitleBar1() {
 * return activeTitleBar1; } public Color getActiveTitleBar2() { return
 * activeTitleBar2; } public Color getBackground() { return background; } public
 * Color getDisabledMask() { return disabledMask; } public Color getForeground()
 * { return foreground; } public Color getPrimary1() { return primary1; } public
 * Color getPrimary2() { return primary2; } public Color getPrimary3() { return
 * primary3; } public Color getPrimaryBorder1() { return primaryBorder1; }
 * public Color getPrimaryBorder2() { return primaryBorder2; } public Color
 * getSecondary1() { return secondary1; } public Color getSecondary2() { return
 * secondary2; } public Color getSecondary3() { return secondary3; } public
 * Color getSecondaryBorder1() { return secondaryBorder1; } public Color
 * getSecondaryBorder2() { return secondaryBorder2; } public Color
 * getTitleBar1() { return titleBar1; } public Color getTitleBar2() { return
 * titleBar2; }
 * 
 * 
 * /TODO: fix overlap bug Container panel = new Container();
 * 
 * final Button left = new Button("Previous"); left.pack(); panel.add(left);
 * 
 * final Button right = new Button("Next"); right.pack();
 * right.setX(left.getX()+5); right.setHeight(left.getHeight());
 * panel.add(right);
 * 
 * panel.setSize(right.getX()+right.getWidth(), left.getHeight());
 * panel.setLocationRelativeTo(display); panel.setY(starty-panel.getHeight()-5);
 * display.add(panel);
 */
