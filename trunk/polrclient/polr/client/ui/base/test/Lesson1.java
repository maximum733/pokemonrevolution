...
import polr.client.ui.base.layout.*;

/**
 * In this lesson we will learn about containers, buttons, labels and layouts.
 * We will create a panel that contains a button and a label and we will give it
 * a RowLayout to evenly space the two child components.
 * 
 * @author davedes
 */
public class Lesson1 extends BasicGame {
	...
	public void init(GameContainer container) throws SlickException {
		...
		content.setLocation(100, 100); // sets panel loc relative to parent
										// (display)
		...
		// give our content a row layout that is aligned (horiz) left and (vert)
		// center.
		...
		label.setHeight(btn.getHeight()); // set same size between the two
											// components
		content.add(label); // add the label to this display so it can be
							// rendered
