...
/**
 * A window that holds a bunch of Sui components. Since this is a component
 * itself (a Frame) it can be reused through multiple demos and tests.
 * 
 * 
 * @author davedes
 */
public class DemoFrame extends Frame {
	...
	public DemoFrame() {
		...
		Image icon = null;
		try {
			icon = new Image("res/skin/shared/warning_icon.png");
		} catch (Exception e) {
		}
		...
		Container glass = createGlass(true, btn.getX(), btn2.getY()
				+ btn2.getHeight() + 5);
		add(glass);

		Component glass2 = createGlass(false, btn.getX(), glass.getY()
				+ glass.getHeight() + 5);
		add(glass2);
		...
		check
				.setLocation(glass2.getX(), glass2.getY() + glass2.getHeight()
						+ 5);
		add(check);
