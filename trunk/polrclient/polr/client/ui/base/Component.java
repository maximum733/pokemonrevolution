...
import polr.client.ui.base.event.ControllerEvent;
import polr.client.ui.base.event.ControllerListener;
...
import polr.client.ui.base.event.MouseWheelEvent;
import polr.client.ui.base.event.MouseWheelListener;
import polr.client.ui.base.skin.ComponentAppearance;

/**
 * An abstract base component which all other components must derive from.
 * 
 * @author davedes
 * @since b.2.0
 */
public abstract class Component {
	...
	/**
	 * Whether this component is ignoring events and letting them pass through
	...
	/**
	 * <p>
	 * This method updates the appearance by retrieving it from the current
	 * skin.
	 * </p>
	 * <p>
	 * This method is typically called from the constructor of a subclass. It is
	 * usually implemented in the following manner:
	 * 
	 * <pre>
	 * &lt;code&gt;public void updateAppearance() {
	 *     setAppearance(Sui.getSkin().getXXXAppearance(this));
	 * }&lt;/code&gt;
	 * </pre>
	 * 
	 * Where <code>getXXXAppearance</code> represents the component appearance
	 * (such as getButtonAppearance).
	 */
	...
	/**
	 * Sets the visibility for this component and all of its children. Invisible
	 * components are not rendered. If a component with the focus is hidden, the
	 * focus is released.
	 * 
	 * @param b
	 *            <tt>true</tt> if it should be renderable
	 */
	 ...
	 * 
	 * @return the parent container, or <tt>null</tt> if this is a top-level
	 *         component
	 */
	...
	/**
	 * Called to render this component. By default, this method will render the
	 * current appearance if it exists and is enabled.
	 * 
	 * @param container
	 *            the GUIContext we are rendering to
	 * @param g
	 *            the Graphics context we are rendering with
	 */
	protected void renderComponent(GUIContext container, Graphics g) {
		...
		if (this.isAppearanceEnabled() && appearance != null) {
			appearance
					.render(container, g, this, Sui.getSkin(), Sui.getTheme());
		}
	}
	 ...
	 * 
	 * @param b
	 *            <tt>true</tt> if this label should be enabled
	 */
	...
	/**
	 * Gets the background color. This will only be drawn if this label is
	 * opaque.
	 * 
	...
	/**
	 * Sets the background color. This will only be drawn if this label is
	 * opaque.
	 * 
	 * @param c
	...
	/**
	 * Sets whether this label is opaque. Opaque components will draw any
	 * transparent pixels with the background color.
	 * 
	 * @param b
	 *            <tt>true</tt> if background color should be drawn
	 */
	...
	/**
	 * Returns whether this label is opaque. Opaque labels will fill a rectangle
	 * of the background color before drawing the text or image.
	 * 
	 * @return <tt>true</tt> if background color should be drawn
	 */
	...
	/**
	 * Called to update this component. By default, this method does nothing.
	 * 
	 * @param container
	 *            the GUIContext we are rendering to
	 * @param delta
	...
	/**
	 * Called to render this component as a whole. For subclasses looking for
	 * custom rendering (outside of skins), it is suggested that you override
	 * renderComponent and disable the appearance.
	 * 
	 * @param container
	 ...
	 * @param topLevel
	 *            <tt>true</tt> if this is a top-level component
	 */
	...
	public void update(GUIContext container, int delta) {
		if (this.isAppearanceEnabled() && appearance != null) {
			appearance.update(container, delta, this, Sui.getSkin(), Sui
					.getTheme());
		}
	...
	/**
	 * Centers this container relative to the specified Component. If the passed
	 * container is <tt>null</tt>, the size of the game context is used instead.
	 * To determine the context, this component must first call getDisplay() --
	 * if that fails, the most recently cached context is used. If no context is
	 * found, the method returns <tt>false</tt> without changing the location.
	 * 
	 * 
	 * @param c
	 *            the component to center with
	 * @return <tt>fale</tt> if no display was found and if this component was
	 *         not moved, <tt>true</tt> otherwise
	 */
	...
	/**
	 * Centers this component based on the size of the given context. If
	 * <tt>null</tt> is passed, no change is made.
	 * 
	 * @param context
	 *            the context to center to
	 */
	public void setLocationRelativeToContext(GUIContext context) {
		if (context == null)
			return;
		setLocation(context.getWidth() / 2f - getWidth() / 2f, context
				.getHeight()
				/ 2f - getHeight() / 2f);
	}

	/**
	 * Sets the x and y positions of this Container, relative to its parent. If
	 * no parent exists, the location is absolute to the GUIContext.
	 * 
	 * 
	 * @param x
	 ...
	 * @see polr.client.ui.base.Container#setBounds(float, float, int,
	 *      int)
	 */
	...
	/**
	 * Sets the x position of this Container, relative to its parent.
	 * 
	 * 
	 * @param x
	 *            the x position of this component
	 * @see polr.client.ui.base.Container#setLocation(float, float)
	 */
	...
	/**
	 * Sets the y position of this Container, relative to its parent.
	 * 
	 * 
	 * @param y
	 *            the y position of this component
	...
	/**
	 * Gets the x position of this Container, relative to its parent.
	 * 
	 * 
	...
	/**
	 * Gets the y position of this Container, relative to its parent.
	 * 
	 * 
	 ...
	 * 
	 * @param x
	 *            the x amount to translate by
	 * @param y
	 *            the y amount to translate by
	 */
	...
	/**
	 * Gets the absolute x position of this component. This is <i>not</i>
	...
	/**
	 * Gets the absolute y position of this component. This is <i>not</i>
	 * relative to the parent's position.
	 * 
	 ...
	 * <p>
	 * The x and y positions are relative to this component's parent. However,
	 * if no parent exists, the x and y positions are equivalent to the absolute
	 * x and y positions.
	 * 
	 * 
	 * @param x
	 ...
	 * 
	 * @param h
	 *            the horizontal amount to adjust
	 * @param v
	 *            the vertical amount to adjust
	 */
	 ...
	 * 
	 * @param b
	 *            <tt>true</tt> if this component should receive key, controller
	...
	public boolean hasFocus() {
		...
		/*
		 * Display d = getDisplay(); return d!=null ? d.getFocusOwner()==this :
		 * false;
		 */
	}

	/**
	 * Releases the focus on this container and all of its Window parents. If
	 * this component did not have the focus, no change is made.
	 * <p>
	 * If the top-level parent of this component is an instance of Window, it
	 * will be deactivated.
	 */
	 ...
	 * @param y
	 *            the y position
	 * @return <tt>true</tt> if this component contains the specified point
	 */
	...
	public boolean inside(float x, float y) {
		...
		return x >= ax && y >= ay && x <= ax + getWidth()
				&& y <= ay + getHeight();
	}

	/**
	 * Glass pane components will ignore events and the underlying components
	 * (ie: the parent panel) will pick them up instead.
	 * <p>
	 * Still testing this.
	 * 
	 * @param b
	 *            whether this component should be glass pane
	 */
	 ...
	 * letting them pass through to underlying components.
	 * 
	...
	/**
	 * Can be overridden if you wish this component's mouse dragged, pressed and
	 * released events to pass down to slick.
	 * <p>
	 * For example, the Display does not (by default) consume events.
	 * 
	 * 
	 * @return <tt>true</tt> if we should consume an even on a mouse
	 *         press/release of this component
	 */
	 ...
	 * 
	 * @param id
	 *            the KeyEvent id constant
	 * @param key
	 *            the Input constant
	 * @param chr
	 *            the character of the key
	 * @see polr.client.ui.base.event.KeyEvent
	 */
	protected void fireKeyEvent(int id, int key, char chr) {
		KeyEvent evt = null;

		final KeyListener[] listeners = (KeyListener[]) listenerList
	...
	/**
	 * Fires the specified mouse event to all mouse listeners in this component.
	 * 
	 * 
	 * @param id
	 *            the MouseEvent id constant
	 * @param button
	 *            the index of the button (starting at 0)
	 * @param x
	 *            the local new x position
	 * @param y
	 *            the local new y position
	 * @param ox
	 *            the local old x position
	 * @param oy
	 *            the local old y position
	 * @param absx
	 *            the absolute x position
	 * @param absy
	 *            the absolute y position
	 * @see polr.client.ui.base.event.MouseEvent
	 */
	protected void fireMouseEvent(int id, int button, int x, int y, int ox,
			int oy, int absx, int absy) {
		MouseEvent evt = null;

		final MouseListener[] listeners = (MouseListener[]) listenerList
	 ...
	 * @see polr.client.ui.base.event.MouseEvent
	 */
	protected void fireMouseEvent(int id, int button, int x, int y, int absx,
			int absy) {
	 ...
	 * @see polr.client.ui.base.event.MouseEvent
	 */
	protected void fireMouseEvent(int id, int x, int y, int ox, int oy,
			int absx, int absy) {
	 ...
	 * 
	 * @param change
	 *            the amount the mouse wheel has changed
	 * @see polr.client.ui.base.event.MouseWheelEvent
	 */
	 ...
	 * 
	 * @param id
	 *            the ControllerEvent id
	 * @param controller
	 *            the controller being used
	 * @param button
	 *            the button that was pressed/released
	 * @see polr.client.ui.base.event.ControllerEvent
	 */
	 ...
	 * 
	 * @param borderVisible
	 *            <tt>true</tt> if a border should be rendered on this
