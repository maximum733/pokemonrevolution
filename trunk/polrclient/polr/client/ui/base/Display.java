...
/**
 * This acts as a top-level component which renders various SuiContainers.
 * 
 * @author davedes
 */
public class Display extends Container {
	...
	/**
	 * Adjusts the size on this display if it has changed from the context. When
	 * a GUIContext (or GameContainer) has changed its size this method should
	 * be called.
	 */
	public void reinit() {
	 ...
	 * 
	 * @param c
	 *            the new owner of the focus, or null if no component has the
	 *            focus
	 */
	...
	/**
	 * Returns the component which currently has the focus. If the current focus
	 * owner is not focusable and/or not visible, the focus owner is released
	...
	public final void render(GUIContext c, Graphics g) {
		...
		if (isShowing() && getWidth() != 0 && getHeight() != 0) {
			...
			if (isClipEnabled()) {
				// TODO: remove?
				/*
				 * clip.x = getAbsoluteX(); clip.y = getAbsoluteY(); clip.width
				 * = getWidth(); clip.height = getHeight();
				 * 
				 * clip.x = 0; clip.y = 0; clip.width = c.getWidth();
		...
		// g.drawString("ToolTipShow: "+(100*tipShowTimer.getPercent())+"%", 10,
		// 55);
		// g.drawString("ToolTipShow: "+(100*tipHideTimer.getPercent())+"%", 10,
		// 70);
	}
	...
	/**
	 * Sets up this display with a new context, changing this display's default
	 * font, width, height, and
	 */
			...
			if (comp != null && comp.isShowing()) {
				if (comp.contains(x, y)) {
					...
					if (comp instanceof Container
						...
						return getDeepestComponentAt((Container) comp, x, y,
								checkGlassPane);
					else {
						return comp;
					}
	...
	private class SlickListener extends InputAdapter {
		...
		public void mouseMoved(int oldX, int oldY, int newX, int newY) {
			// Container comp = getDeepestComponentAt(Display.this, nx, ny,
			// false);
			if (!isShowing()) {
				return;
			}
			...
			Component comp = getDeepestComponentAt(Display.this, newX, newY,
					true);
			if (comp == null) {
				mouseOver = null;
				return;
			 ...
			 * dragComp.fireMouseEvent(MouseEvent.MOUSE_DRAGGED, dragb, nx, ny,
			 * ox, oy, newX, newY); if (dragComp.isConsumingEvents())
			 * input.consumeEvent(); return; }
			 */

			int moveID;
			if (dragging && dragComp.isEnabled()) {
				moveID = MouseEvent.MOUSE_DRAGGED;
				dragComp.fireMouseEvent(moveID, dragb, nx, ny, ox, oy, newX,
						newY);
			...
			if (comp != Display.this && globalEvents
			...
			if (oldComp != mouseOver) {
				if (oldComp != null) {
					oldComp.fireMouseEvent(MouseEvent.MOUSE_EXITED, dragb, nx,
							ny, ox, oy, newX, newY);
				}
				if (mouseOver != null)
					mouseOver.fireMouseEvent(MouseEvent.MOUSE_ENTERED, dragb,
							nx, ny, ox, oy, newX, newY);
			}

			// if the tool tip isn't showing, restart the initial timer since
			// mouse has moved
		...
		public void mousePressed(int button, int x, int y) {
			...
			bdown = true;
			if (comp.isEnabled()) {
				comp.fireMouseEvent(MouseEvent.MOUSE_PRESSED, button, nx, ny,
						x, y);
			}
		...
		public void mouseReleased(int button, int x, int y) {
			...
			if (dragComp != null && dragging) {
				...
				if (dragComp.isEnabled()) {
					dragComp.fireMouseEvent(MouseEvent.MOUSE_RELEASED, button,
							nx, ny, x, y);
				}
				if (dragComp.isConsumingEvents())
					input.consumeEvent();
				if (dragComp != Display.this && globalEvents
						&& Display.this.isEnabled())
					fireMouseEvent(MouseEvent.MOUSE_RELEASED, button, nx, ny,
							x, y);
				dragComp = null;
				return;
			}
			...
			if (comp.isEnabled()) {
				comp.fireMouseEvent(MouseEvent.MOUSE_RELEASED, button, nx, ny,
						x, y);
			}
			if (comp.isConsumingEvents())
				input.consumeEvent();
		...
		public void keyPressed(int key, char c) {
			...
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
		...
		public void controllerButtonPressed(int controller, int button) {
			Component focused = getFocusOwner();
			if (focused != null && focused.isEnabled()) {
				focused.fireControllerEvent(ControllerEvent.BUTTON_PRESSED,
						controller, button);
			}
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
		...
		public void controllerButtonReleased(int controller, int button) {
			Component focused = getFocusOwner();
			if (focused != null && focused.isEnabled()) {
				focused.fireControllerEvent(ControllerEvent.BUTTON_RELEASED,
						controller, button);
			}
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
				fireControllerEvent(ControllerEvent.BUTTON_RELEASED,
						controller, button);
		}
		...
		public void controllerDownPressed(int controller) {
			...
			if (focused != null && focused.isEnabled()) {
				focused.fireControllerEvent(ControllerEvent.BUTTON_PRESSED,
						controller, ControllerEvent.DOWN_BUTTON);
			}
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
				fireControllerEvent(ControllerEvent.BUTTON_PRESSED, controller,
		...
		public void controllerUpPressed(int controller) {
			...
			if (focused != null && focused.isEnabled()) {
				focused.fireControllerEvent(ControllerEvent.BUTTON_PRESSED,
						controller, ControllerEvent.UP_BUTTON);
			}
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
				fireControllerEvent(ControllerEvent.BUTTON_PRESSED, controller,
		...
		public void controllerLeftPressed(int controller) {
			...
			if (focused != null && focused.isEnabled()) {
				focused.fireControllerEvent(ControllerEvent.BUTTON_PRESSED,
						controller, ControllerEvent.LEFT_BUTTON);
			}
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
				fireControllerEvent(ControllerEvent.BUTTON_PRESSED, controller,
		...
		public void controllerRightPressed(int controller) {
			...
			if (focused != null && focused.isEnabled()) {
				focused.fireControllerEvent(ControllerEvent.BUTTON_PRESSED,
						controller, ControllerEvent.RIGHT_BUTTON);
			}
			if (focused != Display.this && globalEvents
					&& Display.this.isEnabled())
				fireControllerEvent(ControllerEvent.BUTTON_PRESSED, controller,
