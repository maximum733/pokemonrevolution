...
public class CheckBox extends ToggleButton {
	...
	public void setHorizontalBoxPosition(int type) {
		if (type != LEADING && type != TRAILING)
			throw new IllegalArgumentException(
					"type must be either LEADING or TRAILING");
	...
	public void pack() {
		...
		setWidth(outer.left + padding.left + objWidth + padding.right
				+ outer.right);
		setHeight(outer.top + padding.top + objHeight + padding.bottom
				+ outer.bottom);
	}
	...
	public Rectangle getAbsoluteBoxBounds() {
		...
		float y = SkinUtil.getObjectY(vert, getAbsoluteY(), getHeight(), pad,
				height);
 ...
 * img = getImage
 * 
 * if img != null if !enabled if selected img getDisabledSelectedImage else img
 * = getDisabledImage else if pressed img = getPressedImage if img == null img =
 * getSelectedImage else if selected if rollover img = getRolloverSelectedImage
 * if img == null img = getSelectedImage else img = getSelectedImage else if
 * rollover img = getRolloverIcon else img = getDefaultImage
 */