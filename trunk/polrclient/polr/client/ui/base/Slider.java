...
public class Slider extends Container implements ScrollConstants {
	 ...
	 *         public SliderAppearance getAppearance() { return
	 *         (SliderAppearance)super.getAppearance(); }
	 */

	/**
	 * Sets the appearance for this slider. If <code>appearance</code> is not an
	 * instance of SliderAppearance, an <code>IllegalArgumentException</code> is
	 * thrown.
	 * 
	 * @param appearance
	...
	public void setAppearance(ComponentAppearance appearance) {
		if (appearance == null)
			throw new IllegalStateException(
					"null appearance not allowed with slider");
		if (!(appearance instanceof SliderAppearance))
			throw new IllegalArgumentException(
					"must pass instance of slider appearance");
		...
		if (orientation == HORIZONTAL) {
			...
			thumbButton.setHeight(Math.min(getHeight(), Math.max(thumbButton
					.getHeight(), minThumbSize)));
			...
			thumbButton.setWidth(Math.min(getWidth(), Math.max(thumbButton
					.getWidth(), minThumbSize)));
	...
	private void updateHeight() {
		if (orientation == HORIZONTAL) {
			...
			float btnHeight = Math
					.max(getHeight() * thumbPercent, minThumbSize);
	...
	protected void fireStateChanged() {
		...
		final ChangeListener[] listeners = (ChangeListener[]) listenerList
	...
	public void setValue(float value) {
		...
		if (old != this.value) {
			if (orientation == HORIZONTAL)
				...
				thumbButton.setY(value
						* (getHeight() - thumbButton.getHeight()));
			fireStateChanged();
		}
	}
	...
	/**
	 * Many skins will choose to highlight the portion of the track that is
	 * selected, whether it be the section BEFORE the thumb, or the section
	 * AFTER it.
	 */
	public Rectangle getAbsoluteTrackSelectionBounds() {
		...
		if (incrementing != 0 && value != 0f && value != 1f) {
			...
			if (orientation == HORIZONTAL) {
				...
				if (incrementing == 1) {
					float th = thumbButton.getX() + thumbButton.getWidth()
							- thumbButton.getWidth() / 2f;
					x += th;
					width = getWidth() - th;
				} else {
				...
				if (incrementing == -1) {
					...
					float th = thumbButton.getY() + thumbButton.getHeight()
							- thumbButton.getHeight() / 2f;
					y += th;
					height = getHeight() - th;
				}
				width = getWidth();
	...
	protected class TrackListener extends MouseAdapter implements
		...
		private void jump(int button, int targetX, int targetY) {
			...
			if (orientation == HORIZONTAL) {
				...
				if (thumbButton.contains(targetX, thumbButton.getAbsoluteY()
						+ thumbButton.getHeight() / 2f)) {
					slideDelay.restart();
					return;
				}
				...
				if (thumbButton.contains(thumbButton.getAbsoluteX()
						+ thumbButton.getWidth() / 2f, targetY)) {
					slideDelay.restart();
					return;
				}
