...
public class ScrollPane extends Container implements ScrollConstants {
	 ...
	 *         public ScrollPaneAppearance getAppearance() { return
	 *         (ScrollPaneAppearance)super.getAppearance(); }
	 */
	...
	public void setAppearance(ComponentAppearance appearance) {
		if (appearance == null)
			throw new IllegalStateException(
					"null appearance not allowed with ScrollPane");
		if (!(appearance instanceof ScrollPaneAppearance)) {
			throw new IllegalArgumentException(
					"appearance must be instance of ScrollPaneAppearance");
		}
		...
		if (a == null) {
			throw new IllegalStateException(
					"no appearance set for ScrollPaneAppearance");
		}
		...
		if (horizontalScrollBar == null || verticalScrollBar == null)
			throw new NullPointerException(
					"null components passed to scroll pane");
		if (horizontalScrollBar.getOrientation() != HORIZONTAL
				|| verticalScrollBar.getOrientation() != VERTICAL)
			throw new IllegalArgumentException(
					"incorrect scroll bar orientation passed to the scroll pane");
	...
	private void updateScrollBarShowing() {
		...
		if (view != null) {
			verticalScrollBar.getSlider().setThumbSize(
	...
	protected boolean isHorizontalScrollBarNeeded() {
		if (horizontalScrollBarPolicy == SCROLLBAR_AS_NEEDED) // only show if
																// it's too big
	...
	protected boolean isVerticalScrollBarNeeded() {
		if (verticalScrollBarPolicy == SCROLLBAR_AS_NEEDED) // only show if it's
															// too big
	...
	protected class VerticalChangeListener implements ChangeListener {
		public void stateChanged(ChangeEvent e) {
			if (view != null) {
				float y = Math.min(view.getHeight(), verticalScrollBar
						.getValue()
						* -view.getHeight());
