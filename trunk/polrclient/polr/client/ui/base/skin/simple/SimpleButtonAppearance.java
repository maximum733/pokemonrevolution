...
public class SimpleButtonAppearance extends SimpleComponentAppearance {
	 ...
	 * button. If <tt>null</tt> is returned, we assume the bounds of the
	 * component (results in a non-rounded rectangle). Most subclasses will use
	 * <tt>0.0f</tt> for size and location, and will only override this for
	 * different rounded corners and number of segments.
	 * 
	...
	/**
	 * Renders a button state (only gradient and border) based on the given
	 * params. This method also checks for ToggleButton instances. If
	 * <tt>aRect</tt> is null, we will assume the bounds of the passed button.
	 * 
	 * 
	 * @param g
	 *            the graphics to render with
	 * @param theme
	 *            the theme we are using
	 * @param btn
	 *            the button to render
	 * @param aRect
	 *            the rectangle we are drawing with, or null
	 * @param grad
	 *            the gradient fill instance to use
	 */
	static void renderButtonState(Graphics g, Theme theme, Button btn,
			Rectangle aRect, GradientFill grad) {
		...
		if (btn instanceof ToggleButton && ((ToggleButton) btn).isSelected()) {
			...
			lightBot = state == Button.ROLLOVER ? theme.getSecondary1() : theme
					.getPrimary3();
		} else {
			switch (state) {
			default:
