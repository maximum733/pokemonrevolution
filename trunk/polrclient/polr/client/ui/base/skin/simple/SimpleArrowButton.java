...
import polr.client.ui.base.*;

/**
 * A Button with an arrow painted on it. The arrow faces in the direction
 * specified (FACE_UP, FACE_LEFT, FACE_DOWN, or FACE_RIGHT). These buttons are
 * typically used in combo boxes and scroll bars. <br />
 * <br />
 * This class also has a utility method,
 * {@link SimpleArrowButton#getScrollButtonAngle(ScrollBar, int)}, which
 * determines the arrow angle based on the scroll bar's orientation and the
 ...
 * 
 * 
 * @author Alexandre Vieira this class is being moved to
 *         mdes.slick.sui.skin.simple.SimpleArrowButton and will be fixed up to
 *         suit nicer aligned arrows
 */
public class SimpleArrowButton extends Button {
	...
	/**
	 * Creates an <code>ArrowButton</code> which arrow faces the specified
	 * angle.
	 * 
	 * @param angle
	 *            The angle the arrow faces.
	 */
	public SimpleArrowButton(float angle) {
		...
		this
				.setSize(ScrollConstants.DEFAULT_SIZE,
						ScrollConstants.DEFAULT_SIZE);
		setPadding(3, 3, 5, 5);
	}
	...
	protected void renderComponent(GUIContext container, Graphics g) {
		...
		/*
		 * float aw = pad.left+(w-pad.right); float ah = pad.top+(h-pad.bottom);
		 * float SIZE = 5; SlickCallable.enterSafeBlock();
		 * 
		 * Sui.getTheme().getForeground().bind();
		 * 
		 * //begin drawing the triangle GL11.glBegin(GL11.GL_TRIANGLES);
		 * 
		 * if (angle==FACE_UP) { GL11.glVertex3f(x+w/2, y+pad.top, 0);
		 * GL11.glVertex3f(x+pad.left, y+SIZE, 0); GL11.glVertex3f(x+, 0); }
		 * GL11.glEnd();
		 * 
