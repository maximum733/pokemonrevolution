...
public class SimpleColorPicker extends Container {
	...
	public SimpleColorPicker() {
		...
		colorHex.setY(colorBox.getY()
				+ (colorBox.getHeight() / 2f - colorHex.getHeight() / 2f));
		svPane.setLocation(colorBox.getX(), colorBox.getY()
				+ colorBox.getHeight() + 3);
		svCursor.setLocation(svPane.getWidth() - svCursor.getWidth() / 2f,
				-svCursor.getWidth() / 2f);

		huePane.setLocation(svPane.getX() + svPane.getWidth() + 8, svPane
				.getY());
		hueCursor.setX(huePane.getX()
				+ (huePane.getWidth() / 2f - hueCursor.getWidth() / 2f));
	...
	private Image tryImage(String ref) {
		try {
			return new ImageUIResource(ref);
		} catch (Exception e) {
			return null;
		}
	}
	...
	public void setSelectedColor(Color color) {
		...
		float huePoint = (huePane.getHeight() - hueCursor.getHeight())
				- hueCursor.getHeight() / 2f + .5f;
		hueCursor.setY(huePane.getY() + huePercent * huePoint);

		// SAT
		svCursor.setX(sat * svPane.getWidth() - svCursor.getWidth()
				+ svCursor.getWidth() / 2f);

		// VAL
		float valPoint = svPane.getHeight() - svCursor.getHeight()
				+ svCursor.getHeight() / 2f;
	...
	private boolean isInsideHuePane(float y) {
		...
		return (y > huePane.getY() - mid)
				&& (y < huePane.getY() + huePane.getHeight() - mid);
	}
	...
	private void hueChanged() {
		if (hueCursor != null) {
			...
			if (isInsideHuePane(y)) {
				float point = hueCursor.getAbsoluteY() - huePane.getAbsoluteY()
						+ mid;
	...
	private static String toHex(Color color) {
		return toHex(color.getRedByte()) + toHex(color.getGreenByte())
				+ toHex(color.getBlueByte());
	}
