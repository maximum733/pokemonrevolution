...
public class SimpleCheckBoxAppearance extends SimpleButtonAppearance {
	...
	public void render(GUIContext ctx, Graphics g, Component comp, Skin skin,
		...
		// also, the outline will only render if we aren't rendering a
		// background
		if (isShowOutline()
				&& (!check.isOpaque() || check.getBackground() == null)) {
		...
		// renders the actual button state for the small box area, using rounded
		// edges
		SimpleButtonAppearance
				.renderButtonState(g, theme, check, btnRect, grad);

		Image def = getDefaultImage();
		if (def != null && check.isSelected()) {
			float x = btnRect.getX()
					+ (btnRect.getWidth() / 2f - def.getWidth() / 2f);
			float y = btnRect.getY()
					+ (btnRect.getHeight() / 2f - def.getHeight() / 2f);
