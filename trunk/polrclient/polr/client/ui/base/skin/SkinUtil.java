...
public class SkinUtil {
	...
	public static boolean installColors(Component c, Color background,
			Color foreground) {
	...
	public static boolean installFont(Component c, Font f) {
		...
		if (compFont == null || compFont == Sui.getDefaultFont()
				|| compFont instanceof FontUIResource) {
			if (f == null)
	...
	public static void renderLabelBase(Graphics g, Label label, String str,
			Image image, float textXOff, float textYOff) {
		...
		if (str != null && str.length() != 0) {
			...
			g.setColor(label.isEnabled() ? label.getForeground() : label
	...
	public static void renderCheckBoxBase(Graphics g, CheckBox check) {
		...
		int mult = check.getHorizontalBoxPosition() == CheckBox.LEADING ? 1
				: -1;
	...
	public static Image getButtonImage(Button button) {
		...
		default:
		case Button.UP:
			return button.isEnabled() ? button.getImage() : button
					.getDisabledImage();
		case Button.DOWN:
	...
	/**
	 * Gets the y position of an object that has the specified height, based on
	 ...
	 * @param hPad
	 *            the horizontal padding (used for left/right align)
	 * @param objWidth
	 *            the width of the object being aligned
	 * @deprecated use getObjectX(Object, int)
	 */
	public static float getObjectX(int align, float x, float width,
			Padding pad, float objWidth) {
		// x position
		switch (align) {
	...
	public static float getObjectY(int align, float y, float height,
			Padding pad, float objHeight) {
		// y position
		switch (align) {
		case Label.CENTER_ALIGNMENT:
		default:
