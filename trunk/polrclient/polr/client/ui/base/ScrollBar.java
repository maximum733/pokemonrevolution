...
public class ScrollBar extends Container implements ScrollConstants {
	 ...
	 * checkOrientation(orientation); this.orientation = orientation;
	 * updateAppearance(); }
	 */
	 ...
	 * 
	 * @return the scroll bar appearance set for this component 1.5 only feature
	 *         public ScrollBarAppearance getAppearance() { return
	 *         (ScrollBarAppearance)super.getAppearance(); }
	 */
	...
	public void setAppearance(ComponentAppearance appearance) {
		if (appearance == null)
			throw new IllegalStateException(
					"null appearance not allowed with ScrollBar");
		if (!(appearance instanceof ScrollBarAppearance)) {
			throw new IllegalArgumentException(
					"appearance must be instance of ScrollBarAppearance");
		}
		...
		if (a == null) {
			throw new IllegalStateException(
					"no appearance set for this scroll bar");
		}
		...
		if (incButton == null || decButton == null || slider == null)
			throw new NullPointerException(
					"null components passed to scroll bar");
		if (slider.getOrientation() != orientation)
			throw new IllegalArgumentException(
					"passed slider doesn't match orientation of scroll bar");
	 ...
	 * @see polr.client.ui.base.ScrollConstants#INCREMENT
	 */
	 ...
	 * @see polr.client.ui.base.ScrollConstants#DECREMENT
	 */
	...
	private void updateWidth() {
		...
		if (orientation == HORIZONTAL) {
			...
			slider.setWidth(getWidth()
					- (incButton.getWidth() + decButton.getWidth()));
		} else { // VERTICAL
	...
	private void updateHeight() {
		...
		if (orientation == HORIZONTAL) {
			...
			slider.setHeight(getHeight()
					- (incButton.getHeight() + decButton.getHeight()));
		}
	}
	...
	public void scrollByUnit(int direction) {
		...
		if (orientation == VERTICAL) {
			direction *= -1;
			size = scrollSpace + slider.getThumbButton().getHeight()
					/ slider.getHeight();
		} else
			size = scrollSpace + slider.getThumbButton().getWidth()
					/ slider.getWidth();
	...
	protected class IncDecListener implements ActionListener,
			MouseWheelListener {
		public void actionPerformed(ActionEvent e) {
			int direction = (e.getSource() == incButton) ? INCREMENT
					: DECREMENT;
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			...
			int direction = (amt > 0) ? INCREMENT : DECREMENT;
			scrollByUnit(direction);
		}
	}
}
