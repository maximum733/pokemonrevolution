...
public class TextArea extends TextComponent {
		...
		if (wrapDirty) {
			...
			if (!isWrapEnabled()) {
				...
				if (resizing) {
					...
					setWidth(Math.max(getMinimumSize().width, pad.left + width
							+ pad.right));
				}
				return;
			}
			...
			while (value.length() > 0) {
				...
				offset += lastLineLen;
				lines.add(new Line(aLine, yoff, width, defaultLineHeight,
						offset));
			...
			float maxHeight = pad.top + defaultLineHeight * lines.size()
					+ pad.bottom;
			if (resizing) {
				setHeight(Math.max(getMinimumSize().height, maxHeight));
			}
	...
	public class Line {
		...
		protected Line(String str, float yoff, float width, float height,
				int offset) {
			this.str = str;
			this.yoff = yoff;
			this.width = width;
	...
	/*
	 * protected void drawWrappedString(Graphics g, String text) { Color
	 * oldColor = g.getColor(); Font oldFont = g.getFont();
	 * 
	 * g.setFont(getFont());
	 ...
	 * int pad = getPadding();
	 * 
	 * float x = getAbsoluteX()+pad; float y = getAbsoluteY()+pad;
	 * 
	 * int maxWidth = getWidth()-pad; int height = getHeight()-pad;
	 * 
	 * Vector lines = new Vector(); while (text.length() > 0) { String aLine =
	 * text; while (getFont().getWidth(aLine) > maxWidth) { int rSpace =
	 * aLine.lastIndexOf(" "); if (rSpace==-1) { break; } aLine =
	 * aLine.substring(0, rSpace); } lines.add(aLine); text =
	 * text.substring(aLine.length()).trim(); }
	 * 
	 * final int numLines = lines.size(); final int lineHeight =
	 * f.getHeight(" "); final float startY = y;
	 * 
	 * for( int i = 0; i < numLines; i++ ) { String line =
	 * (String)lines.elementAt( i ); float width = f.getWidth(line); float
	 * thisLineX = x; int yoff = getYOffset(line); g.drawString(line,
	 * (int)thisLineX, (int)(startY+(ilineHeight-yoff))); }
	 * 
	 * g.setFont(oldFont); g.setColor(oldColor); }
	 */
		...
		public void keyPressed(KeyEvent e) {
			...
			if (key == Input.KEY_ENTER) {
				...
				if (index > 0) {
					...
					int preIndex = Math.max(0, Math.min(
							pre.str.trim().length(), currentIndex));
				...
				if (index >= 0 && index < getLineCount() - 1) {
					...
					// ^ index 1
					int currentIndex = caretPos - line.offset;
					int nextIndex = Math.max(0, Math.min(next.str.trim()
							.length(), currentIndex));
