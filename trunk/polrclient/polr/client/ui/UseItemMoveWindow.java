...
public class UseItemMoveWindow extends Frame {
	...
	public UseItemMoveWindow(final String packet, OurPokemon poke) {
		...
		for (int i = 0; i < poke.getMoves().length; i++) {
			if (poke.getMoves()[i] != null
					&& !poke.getMoves()[i].equalsIgnoreCase("null")) {
				...
				case 0:
					moves[i].addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							GlobalGame.getPacketGenerator().write(packet + 0);
							close();
						}
					});
					break;
				case 1:
					moves[i].addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							GlobalGame.getPacketGenerator().write(packet + 1);
							close();
						}
					});
					break;
				case 2:
					moves[i].addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							GlobalGame.getPacketGenerator().write(packet + 2);
							close();
						}
					});
					break;
				case 3:
					moves[i].addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							GlobalGame.getPacketGenerator().write(packet + 3);
							close();
						}
					});
					break;
