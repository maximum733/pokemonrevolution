...
import polr.client.ui.ServerSelector;
import polr.client.ui.StarterChoices;
import polr.client.ui.ToSWindow;
...
public class LoginScreen extends Window {
	...
	private void initGUI() {
		...
		try {
			...
			passLabel.setFont(GlobalGame.getDPFontSmall());
			...
			login.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (regConfirmPass.isVisible())
						...
						GlobalGame.setUser(authUser.getText());
			...
			if (now.get(Calendar.MONTH) >= 11) {
				if (now.get(Calendar.DAY_OF_MONTH) > 20
						&& now.get(Calendar.DAY_OF_MONTH) < 28) {
					GlobalGame.getSoundPlayer().playChannel("robotchristmas",
							"music");
				} else {
					GlobalGame.getSoundPlayer().playChannel("opening", "music");
				}
			} else {
				GlobalGame.getSoundPlayer().playChannel("opening", "music");
			}
	...
	private void authDeleteActionPerformed(ActionEvent evt) {
		if (authUser.getText() == "") {
			GlobalGame.messageBox("You must enter a username.", this);
		} else if (authPass.getText() == "") {
			GlobalGame.messageBox("You must enter a password.", this);
		} else {
	...
	private boolean validData() {
		if (starterChoices.getChoice() != -1) {
			if (!authPass.getText().equals("")
					&& authPass.getText().equals(regConfirmPass.getText())) {
				if (isEmailValid(regEmail.getText())) {
					if (!authUser.getText().equals("")) {
						if (!authUser.getText().contains(" ")) {
							...
							GlobalGame
									.messageBox(
											"You may not have spaces in your username.",
											this);
						}
					} else {
						GlobalGame.messageBox("Type in your username.", this);
					}
				} else {
					GlobalGame.messageBox("Invalid e-mail address.", this);
				}
			} else {
				GlobalGame.messageBox(
						"Your password must be typed in both boxes.", this);
			}
		} else {
			GlobalGame.messageBox(
					"Make sure your Pokemon is selected correctly.", this);
		}
		return false;
	}
	...
	private void register() {
		if (validData()) {
			System.out.println(authUser.getText() + " " + authPass.getText()
					+ " " + starterChoices.getChoice() + " "
					+ regEmail.getText());
			if (female.isSelected())
				packetGen.register(authUser.getText(), authPass.getText(),
						starterChoices.getChoice(), 12, regEmail.getText());
			else
				packetGen.register(authUser.getText(), authPass.getText(),
						starterChoices.getChoice(), 29, regEmail.getText());
		}
	}
	...
	public void enterKeyDefault() {
		if (!tos.isVisible() && login.isEnabled() && register.isEnabled()) {
			if (!regConfirmPass.isVisible()) {
				login.setEnabled(false);
				register.setEnabled(false);
				GlobalGame.setUser(authUser.getText());
	...
	public void tabKeyDefault() {
		if (!tos.isVisible()) {
			if (!regConfirmPass.isVisible()) {
				if (authUser.hasFocus()) {
					authUser.releaseFocus();
					authPass.grabFocus();
				} else if (authPass.hasFocus()) {
					authPass.releaseFocus();
					authUser.grabFocus();
				} else {
					authUser.grabFocus();
				}
				...
				if (authUser.hasFocus()) {
					...
					authPass.releaseFocus();
					regConfirmPass.grabFocus();
				} else if (regConfirmPass.hasFocus()) {
					regConfirmPass.releaseFocus();
					regEmail.grabFocus();
				} else if (regEmail.hasFocus()) {
					regEmail.releaseFocus();
					authUser.grabFocus();
				} else {
					authUser.grabFocus();
				}
