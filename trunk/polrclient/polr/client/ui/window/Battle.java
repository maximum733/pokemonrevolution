...
public class Battle extends Frame {
	...
	private Image buttonEnabled, buttonPressed, buttonSmall,
			buttonSmallPressed;
	...
	public Battle(String playerName, PacketGenerator p,
			OurPokemon[] playerPokes, int playerInd, GlobalGame game) {
		thisGame = game;
		packetGen = p;
	...
	public void initComponents() {
		...
		try {
			...
			buttonSmallPressed = new Image(
					"res/battle/button_small_pressed.png");
		} catch (SlickException e2) {
			// TODO Auto-generated catch block
			e2.printStackTrace();
		}
		...
		f7.setVisible(true);
		f7.setSize(82, 48);
		f7.setLocation(f5.getX() + f5.getWidth() + 2, f5.getY()
				+ (f5.getHeight() / 2));
	...
	public void updatePP(int move, int pp) {
		 ...
		 * pp1.setText(getCurPoke().getMovecurPP()[0] + "/" +
		 * getCurPoke().getMovemaxPP()[0]); if
		 * (!getCurPoke().getMoves()[1].equals("null"))
		 * pp2.setText(getCurPoke().getMovecurPP()[1] + "/" +
		 * getCurPoke().getMovemaxPP()[1]); if
		 * (!getCurPoke().getMoves()[2].equals("null"))
		 * pp3.setText(getCurPoke().getMovecurPP()[2] + "/" +
		 * getCurPoke().getMovemaxPP()[2]); if
		 * (!getCurPoke().getMoves()[3].equals("null"))
		 * pp4.setText(getCurPoke().getMovecurPP()[3] + "/" +
		 * getCurPoke().getMovemaxPP()[3]);
		 */
	}
	...
	public void addEnemyPokemon(String pokemonName, int index, int level,
			int gender, int hp, String species) {
		...
		enemyPokes[index].setSpecies(enemyPokes[index].getSpecies().valueOf(
				species));
	...
	public void drawBattleData() {
		...
		playerInfo.setText(getCurPoke().getSpecies().name() + "  Lv:"
				+ getCurPoke().getLevel());
		playerInfo.setForeground(Color.white);
		playerInfo.setBounds(152, 109, 94, 12);

		// display enemy's data
		enemyInfo.setText(getCurEnemyPoke().getSpecies().name() + "  Lv:"
	...
	public void updatePlayerHP(int newValue) {
		...
		} else if (getCurPoke().getCurHP() < getCurPoke().getMaxHP() / 2
				&& getCurPoke().getCurHP() > getCurPoke().getMaxHP() / 3) {
	...
	public void updateEnemyHP(int newValue) {
		...
		} else if (newValue < getCurEnemyPoke().getMaxHP() / 2
				&& getCurEnemyPoke().getCurHP() > getCurEnemyPoke().getMaxHP() / 3) {
	...
	public void giveExp(String a) {
		...
		if (info[2] == "Y") {
			addMessage(ourPokes[Integer.parseInt(info[0])].getName()
					+ " leveled up!");
		}
