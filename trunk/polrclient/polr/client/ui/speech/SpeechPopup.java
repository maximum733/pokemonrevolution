...
public class SpeechPopup extends Frame {
	...
	/*
	 * //commented out because it's too confusing public void advance() {
	 * 
	 * triangle = null;
	 * 
	 * if (animAction == null) { if (canAdvance()) { speechDisplay.setText("");
	 * if (stringToPrint != null) advancedPast(stringToPrint); stringToPrint =
	 * speechQueue.poll();
	 * 
	 * if (stringToPrint != null) { animAction = new TimerTask() { public void
	 * run() { if (speechDisplay.getText().equals(stringToPrint)) { animAction =
	 * null; try { cancel(); } catch (IllegalStateException e) { }
	 * triangulate(); } else { try {
	 * speechDisplay.setText(stringToPrint.substring(0,
	 * speechDisplay.getText().length() + 1)); } catch
	 * (StringIndexOutOfBoundsException e) {
	 * speechDisplay.setText(stringToPrint); } } }}; try {
	 * printingTimer.schedule(animAction, 0, 30);} catch (Exception e) {
	 * animAction = null; e.printStackTrace(); }
	 * 
	 * advancing(stringToPrint); } } } else { speechDisplay.setText("");
	 * animAction.cancel(); animAction = null;
	 * speechDisplay.setText(stringToPrint); triangulate(); } }
	 */
