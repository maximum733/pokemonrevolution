...
/*
 * This UI displays your party, but differs in that it exists so the user can swap pokemon between box slots and the party.
 */
public class TeamForBox extends Container {
	...
	public TeamForBox(OurPokemon[] ourPokes, int boxNum, int boxInd,
			PacketGenerator out) {
		boxNumber = boxNum;
		boxIndex = boxInd;
		packetGen = out;
	...
	public void initGUI() {
		...
		for (int i = 0; i < 6; i++) {
			...
			try {
				...
				pokes[i].add(level[i]);
				level[i].setLocation(pokeName[i].getX()
						+ pokeName[i].getWidth() + 10, 5);
				hp[i].setSize(114, 10);
				hp[i].setLocation(40, pokeName[i].getY()
						+ pokeName[i].getHeight() + 5);
				pokes[i].add(hp[i]);
