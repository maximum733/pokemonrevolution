...
public class GameMap extends TiledMap {
	...
	public boolean isColliding(Player thisPlayer, Dirs dir) {
		...
		int colTileID = getLayer("Collisions").getTileID(newX / 32,
				(newY + 8) / 32);
		int watTileID;
		// Not all maps have a water layer
		/*
		 * try { watTileID = getLayer("Water").getTileID( newX / 32, (newY + 8)
		 * / 32); } catch (Exception e) { watTileID = 0; }
		 */
		// Not all maps have a ledges layer
		int ledTileID = 0;
		try {
			if (thisPlayer.facing != Dirs.Right) {
				ledTileID = getLayer("LedgesRight").getTileID(newX / 32,
						(newY + 8) / 32);
			}
			if (ledTileID == 0 && thisPlayer.facing != Dirs.Left) {
				ledTileID = getLayer("LedgesLeft").getTileID(newX / 32,
						(newY + 8) / 32);
			}
			if (ledTileID == 0 && thisPlayer.facing != Dirs.Down) {
				ledTileID = getLayer("LedgesDown").getTileID(newX / 32,
						(newY + 8) / 32);
			}
		} catch (Exception e) {
			ledTileID = 0;
		}
		if (colTileID + ledTileID != 0 || getNPCAt(newX, newY) != null)
			return true;
		return false;
	...
	public boolean isNewMap(Player thisPlayer, Dirs dir) {
		...
		if (newX < 0 || newX >= getWidth() * 32 || newY < -8
				|| newY + 8 >= getHeight() * 32)
			return true;
		else
			return false;
	...
	public boolean isRendering() {
		if (rendering) {
			...
			if (!(drawWidth < 0 && getXOffset() < 0 || drawWidth > 832
					&& getXOffset() > 832)
					&& !(drawHeight < 0 && getYOffset() < 0 || drawHeight > 632
							&& getYOffset() > 632)) {
				return true;
			}
		}
	...
	public void setXOffset(int offset) {
		...
		if (map != null)
			map.auxiliarySetXOffset(offset - map.getWidth() * 32
					- getXOffsetModifier() + map.getXOffsetModifier());

		map = matrix.getMap(mapX + 1, mapY);
		if (map != null)
			map.auxiliarySetXOffset(offset + getWidth() * 32
					- getXOffsetModifier() + map.getXOffsetModifier());
				...
				if (layer == walkableLayer) {
					for (Player p : mapPlayers) {
						if ((p.y >= mapY * 32 - 39) && (p.y <= mapY * 32 + 32)
								&& (p.curImg != null))
							graphics.drawString(p.username, xOffset
									+ (p.x - (graphics.getFont().getWidth(
											p.username) / 2)) + 14, yOffset
									+ p.y - 36);
					}
				}
				graphics.scale(2, 2);
