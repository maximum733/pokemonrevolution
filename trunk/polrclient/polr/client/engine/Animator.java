...
public class Animator {
	...
	// Sets up calls
	public Animator(GameMapMatrix maps, GlobalGame g) {
		animTimer = new Timer();
	...
	private void animatePlayer(Player p) {
		...
		if (p.thisPlayer) {
			if (p.x > p.svrX) {
				p.map.setXOffset((p.map.getXOffset() + ANIMATION_INCREMENT)); // +
																				// p.map
				// .getXOffsetModifier()));
			} else if (p.x < p.svrX) {
				p.map.setXOffset((p.map.getXOffset() - ANIMATION_INCREMENT)); // +
	...
	// sets client as game
	void setThisGame(GlobalGame thisGame) {
		this.thisGame = thisGame;
	}

	// gets client
	GlobalGame getThisGame() {
		return thisGame;
	}
}
