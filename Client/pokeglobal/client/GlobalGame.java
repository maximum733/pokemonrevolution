...
import pokeglobal.client.engine.Animator;
import pokeglobal.client.engine.GameMap;
import pokeglobal.client.engine.GameMapMatrix;
...
import pokeglobal.client.network.ProtocolHandler;
import pokeglobal.client.network.ProxiedProtocolHandler;
import pokeglobal.client.network.security.diskUtilities;
import pokeglobal.client.sound.SoundManager;
import pokeglobal.client.ui.ChatWindow;
import pokeglobal.client.ui.PartyInfo;
import pokeglobal.client.ui.PokeStorageBoxPopup;
import pokeglobal.client.ui.ShopWindow;
import pokeglobal.client.ui.base.Container;
import pokeglobal.client.ui.base.Display;
import pokeglobal.client.ui.base.MessageBox;
import pokeglobal.client.ui.speech.NPCSpeechPopup;
import pokeglobal.client.ui.window.Battle;
import pokeglobal.client.ui.window.IntroScreen;
import pokeglobal.client.ui.window.LoadingScreen;
import pokeglobal.client.ui.window.LoginScreen;
import pokeglobal.client.ui.window.MainInterface;
...
public class GlobalGame extends BasicGame {
	...
	public static void main(String args[]) {
		try {
			...
			container.setDisplayMode(width, height, Boolean.parseBoolean(options.get("fullScreen")));
			container.start();
		} catch (Exception e) {
	...
	public void usePokeStorageBox(String pokeList) throws SlickException {
		PokeStorageBoxPopup storageBoxPopup = new PokeStorageBoxPopup(packetGen, pokeList, thisPlayer.getTeam());
	...
	public void keyPressed(int key, char c) {
		...
		if (isPlaying && thisPlayer != null
			...
			if (!mainInterface.isChatting() && (key == (Input.KEY_RSHIFT))) {
				...
				if (thisPlayer.facing == Dirs.Up) {
					packetGen.write("TT" + GlobalGame.CHARSEP + thisPlayer.x + GlobalGame.CHARSEP + (thisPlayer.y - 32));
				} else if (thisPlayer.facing == Dirs.Down) {
					packetGen.write("TT" + GlobalGame.CHARSEP + thisPlayer.x + GlobalGame.CHARSEP + (thisPlayer.y + 32));
				} else if (thisPlayer.facing == Dirs.Left) {
					packetGen.write("TT" + GlobalGame.CHARSEP + (thisPlayer.x - 32) + GlobalGame.CHARSEP + thisPlayer.y);
				} else if (thisPlayer.facing == Dirs.Right) {
					packetGen.write("TT" + GlobalGame.CHARSEP + (thisPlayer.x + 32) + GlobalGame.CHARSEP + thisPlayer.y);
				}
			}
