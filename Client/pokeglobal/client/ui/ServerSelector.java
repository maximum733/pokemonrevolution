...
/*		try {
			BufferedReader f = new BufferedReader(new FileReader("pokeglobal/client/network/servers.txt"));
			for(int i = 0; i < 5; i++) {
				listedServers[i] = f.readLine();
			}
		}
		catch (Exception e) {
			e.printStackTrace();
...
*/	
		listedServers[0] = "pokeglobal.dyndns.org";
		...
		server1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				if(!proxyName.getText().equalsIgnoreCase("") && !proxyPort.getText().equalsIgnoreCase("")) {
					GlobalGame.setProxy(proxyName.getText(), Integer.parseInt(proxyPort.getText()));
				}
