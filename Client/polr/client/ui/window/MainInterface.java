...
public class MainInterface extends Frame {
	...
	public MainInterface(GameClient game) {
		thisGame = game;
		initGUI();
	}
	...
	public void initGUI() {
		...
		helpInfo.add("");
		helpInfo.add("There are Game Moderators everywhere, they're more than willing to help.");
		helpInfo.add("For more help, go to www.pokeglobal.org and post in the forums Help Section");
		
		localChat = new ArrayList<String>();
		worldChat = new ArrayList<String>();
		charL = 0; chatLocation = 0;
		...
		chatList.setFont(GameClient.getDPFontSmall());
		...
		current = new Label();
		current.setFont(GameClient.getDPFontSmall());
		...
		try {
			...
			requests.setDownImage(new Image("res/ui/recuestsPressed_32x32.png"));
		} catch (SlickException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			...
			pokegear.setDownImage(new Image("res/ui/pokegearPressed_32x32.png"));
		} catch (SlickException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
	...
	public void goToHelp() {
		...
		for(int i = charL; i < helpInfo.size() && chatList.getLineCount() <
				chatList.getHeight() / GameClient.getDPFontSmall().getLineHeight() && chatList.getHeight() < 280; i++) {
			chatList.setText(chatList.getText().equalsIgnoreCase("") ? helpInfo.get(i): chatList.getText() + "\n" + helpInfo.get(i));
		}
		scrollUp.setVisible(false);
		scrollDown.setVisible(false);
		autoScroll.setVisible(false);
	...
	public void addWorldChatLine(String newChat) {
		...
		newChat = newChat.substring(0, endex + 1) + 
			newChat.substring(endex + 1).replace(">", "=>= ")
									.replace("<", "=<=");
	...
	private void reloadChat() {
		...
		case 0:
			chatList.setText("");
			chatList.setCaretPosition(0);
			for(int i = charL; i < worldChat.size() && chatList.getLineCount() <
					chatList.getHeight() / GameClient.getDPFontSmall().getLineHeight() && chatList.getHeight() < 280; i++) {
				chatList.setText(chatList.getText().equalsIgnoreCase("") ? worldChat.get(i) : chatList.getText() + "\n" + worldChat.get(i));
			}
			break;
		case 1:
			chatList.setText("");
			chatList.setCaretPosition(0);
			for(int i = charL; i < localChat.size() && chatList.getLineCount() <
					chatList.getHeight() / GameClient.getDPFontSmall().getLineHeight() && chatList.getHeight() < 280; i++) {
				chatList.setText(chatList.getText().equalsIgnoreCase("") ? localChat.get(i) : chatList.getText() + "\n" + localChat.get(i));
			}
			break;
		}
	}
